...
var WAI_ARIA_CalendarMonthObject = function(_Settings) {
/*
...
*/
	...
	********************************************************************************/
	...
	var This = this
		...
		,_hasToDateLimit = false //shows if _dateLimitTo has been set, so you do not need to do the more expencive "instanceof" check when needed
	;
	...
	*/
		var hasDateBeenLimted = false, obj, errorDate, activeDateYear, activeDateMonth, hasCoodrinates;		
		...
		if(_hasFromDateLimit) {
			if(_activeDate < _dateLimitFrom) {
				_activeDate = new Date(_dateLimitFrom.getFullYear(), _dateLimitFrom.getMonth(), _dateLimitFrom.getDate());
				hasDateBeenLimted = true;
			} else if(activeDateYear == _dateLimitFrom.getFullYear() && activeDateMonth == _dateLimitFrom.getMonth()) {
		...
		if(_hasToDateLimit) {
			if(_activeDate > _dateLimitTo) {
				_activeDate = new Date(_dateLimitTo.getFullYear(), _dateLimitTo.getMonth(), 1);
				hasDateBeenLimted = true;
			} else if(activeDateYear == _dateLimitTo.getFullYear() && activeDateMonth == _dateLimitTo.getMonth()) {
	...
	*/
		...
		limitToTimeStamp = _hasToDateLimit ? _dateLimitTo.getTime() : null;
		...
		if(_hasArrowLabel) {
			...
			for(rowIndex = 0; rowIndex < 6; rowIndex++) {
				for(cellIndex = 0; cellIndex < 7; cellIndex++) {
					...
					if(_hasToDateLimit && obj.timeStamp == limitToTimeStamp) {
	...
	*/
		...
		for(i = 0; i < 6; i++)
		{
			...
			for(j = 0; j < 7; j++)
			{
				...
				if(_hasFromDateLimit && _dateLimitFrom && _dateLimitFrom > date || _hasToDateLimit && _dateLimitTo && _dateLimitTo < date) {
					isDisabled = true;
				} else {
					isDisabled = false;
				}
	...
	*/
		...
		if(_hasToDateLimit && dateTimestamp == _dateLimitTo.getTime()) {
			strLabel += _Settings.Phrases.endOfPeriod + ', ';
		}
		
		ret.Button = new WAI_ARIA_ButtonObject({
			Phrases : {
				hiddenButtonText :  _longMonthNameArr[date.getMonth()] + ' ' + date.getDate() + ' ' + date.getFullYear() + ', ' + _longWeekNameArr[date.getDay()]
				,visualButtonText : typeof _Settings.numberFormatCall == 'function' ? _Settings.numberFormatCall(date.getDate()) : date.getDate()
				,label : strLabel
			}
			, $root : $cell
