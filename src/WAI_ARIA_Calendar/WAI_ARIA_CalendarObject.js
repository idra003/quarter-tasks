...
var WAI_ARIA_CalendarObject = function(_Settings) {
/*
	Generates and maintains the WAI-ARIA Calendar object i.e. basic navigation 
*/
	var
		CLASS_NAME = 'EurolandTools-CommonObjects-Calendar'
	...
	********************************************************************************/
	...
	var This = this
		,_DefaultSettings = {
			...
			, iconURL : '' //depricated, moved to CSS
			...
			, dateSelectionCall : null //the method that will be called when a date is selected, the Date will be passed as the first variable
			
			, isDropDown : true //if FALSE the calendar is a fully visible calendar entered into the DOM else is the standard drop down calendar
			...
			, numberFormatCall : null //the method that is called when a custom number display is neede ex. arabic number
			
			, instanceName : '' //an irrelavent variable to allow to identify the instance for debugging purposes
		}
		...
		,_hasFadeOutClose = false //if set to TRUE the close will only fade out the calendar and not move it
	;
	...
	function _init() {
		...
		if(_Settings.iconURL.length) {
			throw('WAI_ARIA_CalendarObject::_init() - _Settings.iconURL is a depricated setting, the icon was moved to CSS.');
		}
		...
		if(typeof WAI_ARIA_CalendarObject.prototype.calendarCount == 'number') {
			WAI_ARIA_CalendarObject.prototype.calendarCount++;
		} else {
			WAI_ARIA_CalendarObject.prototype.calendarCount = 1;
		}
		
		INSTANSE_NR = WAI_ARIA_CalendarObject.prototype.calendarCount;
		...
		_PrevYearButton = new WAI_ARIA_ButtonObject({
			Phrases : {
		...
		_PrevMonthButton = new WAI_ARIA_ButtonObject({
			Phrases : {
				hiddenButtonText : 'N/A'
				, visualButtonText : '‹'
		...
		_NextMonthButton = new WAI_ARIA_ButtonObject({
			Phrases : {
				hiddenButtonText : 'N/A'
				, visualButtonText : '›'
		...
		_NextYearButton = new WAI_ARIA_ButtonObject({
			Phrases : {
				hiddenButtonText : 'N/A'
				, visualButtonText : '»'				
		...
		_CurrentMonthButton = new WAI_ARIA_ButtonObject({
			Phrases : {
		...
		objCalendarDefaultSettings = {
			...
			, numberFormatCall : _Settings.numberFormatCall
			
			, Phrases : {
				sunShort : _Settings.Phrases.sunShort
				, monShort : _Settings.Phrases.monShort
				, tueShort : _Settings.Phrases.tueShort
				, wedShort : _Settings.Phrases.wedShort
				, thuShort : _Settings.Phrases.thuShort
				, friShort : _Settings.Phrases.friShort
				, satShort : _Settings.Phrases.satShort
				
				, sunday : _Settings.Phrases.sunday
				, monday : _Settings.Phrases.monday
				, tuesday : _Settings.Phrases.tuesday
				, wednesday : _Settings.Phrases.wednesday
				, thursday : _Settings.Phrases.thursday
				, friday : _Settings.Phrases.friday
				, saturday : _Settings.Phrases.saturday
				
				, january : _Settings.Phrases.january
				, february : _Settings.Phrases.february
				, march : _Settings.Phrases.march
				, april : _Settings.Phrases.april
				, may : _Settings.Phrases.may
				, june : _Settings.Phrases.june
				, july : _Settings.Phrases.july
				, august : _Settings.Phrases.august
				, september : _Settings.Phrases.september
				, october : _Settings.Phrases.october
				, november : _Settings.Phrases.november
				, december : _Settings.Phrases.december

				, useArrowKeys : _Settings.Phrases.useArrowKeys
				, startOfPeriod : _Settings.Phrases.startOfPeriod
				, endOfPeriod : _Settings.Phrases.endOfPeriod
			}
		};
		
		_MonthA = new WAI_ARIA_CalendarMonthObject(objCalendarDefaultSettings);
		_MonthB = new WAI_ARIA_CalendarMonthObject(objCalendarDefaultSettings);
		...
		_ExtraPrevButton = new WAI_ARIA_ButtonObject({
			Phrases : {
				hiddenButtonText : ''
				, visualButtonText : '‹'				
			}
		...
		_ExtraNextButton = new WAI_ARIA_ButtonObject({
			Phrases : {
				hiddenButtonText : ''
				, visualButtonText : '›'				
			}
		...
		_ExtraCurrentButton = new WAI_ARIA_ButtonObject({
		...
		_ExtraPanelCloseButton = new WAI_ARIA_ButtonObject({
			Phrases : {
		...
		_CloseButton = new WAI_ARIA_ButtonObject({
			Phrases : {
				visualButtonText : _Settings.Phrases.close
				,hiddenButtonText : ''
		...
		if(_Settings.isDropDown) {
			This.$root
				.append($(document.createElement('div')).addClass(CLASS_NAME + '-Selection')
					...
					.append($(document.createElement('span')).addClass(CLASS_NAME + '-CalendarIcon'))				
					.bind('click', selectionIndicatorClick)
	...
	/*
		...
		dDate - [Optional] Date, The date of the selected day
	*/
		var date;
		
		if(dDate instanceof Date) {
			
			date = new Date(dDate.getFullYear(), dDate.getMonth(), dDate.getDate());
		
			if(_dateLimitFrom instanceof Date) {
				if(date < _dateLimitFrom) {
					throw('[WAI_ARIA_CalendarObject].setSelectedDay() - The set date is in the disabled date range [' + date + '] _dateLimitFrom:[' + _dateLimitFrom + ']' );
					return;
				}
			}
			
			if(_dateLimitTo instanceof Date) {
				if(date > _dateLimitTo) {
					throw('[WAI_ARIA_CalendarObject].setSelectedDay() - The set date is in the disabled date range [' + date + '] _dateLimitFrom:[' + _dateLimitTo + ']' );
					return;
				}
			}
			
			setMonth(ANIMATE_MONTHS_NONE, date);
			selectDay(date);
	...
	*/
		...
		_$selectionIndicatorText.text(_Settings.Phrases.noDateSelected);		
		_$hiddenButton.text(_Settings.Phrases.noDateSelected);
		_selectedDate = null;
		
		_MonthA.clearSelection();
		_MonthB.clearSelection();		
		
		if(_dateLimitTo instanceof Date) {
			setMonth(ANIMATE_MONTHS_NONE, _dateLimitTo);
		} else if(_dateLimitFrom instanceof Date) {
			setMonth(ANIMATE_MONTHS_NONE, _dateLimitFrom);
		} else {
			dDate = new Date();
			dDate = new Date(dDate.getFullYear(), dDate.getMonth(), dDate.getDate());
			setMonth(ANIMATE_MONTHS_NONE, dDate);
		}
	...
	*/
		...
		if(_dateLimitFrom instanceof Date) {
			if(dDate < _dateLimitFrom) {
				return;
			}
		}
		
		if(_dateLimitTo instanceof Date) {
			if(dDate > _dateLimitTo) {
				return;
			}
		}
		...
		if(typeof _Settings.dateSelectionCall == 'function') {
			_Settings.dateSelectionCall(_selectedDate);
		}
	}
	...
	*/		
		var Button = new WAI_ARIA_ButtonObject({
	...
	*/
		...
		_ExtraCurrentButton.setText(
			typeof _Settings.numberFormatCall == 'function' ? _Settings.numberFormatCall('' + year) : '' + year
	...
	*/
		var fromYear, toYear, ret;
		
		fromYear = Math.floor(year / 10) * 10;
		toYear = fromYear + 10;
		//fromYear--;
		
		ret = _Settings.Phrases.fromTo.replace('{0}', fromYear).replace('{1}', toYear);
		// if(typeof _Settings.numberFormatCall == 'function') {
			// ret = _Settings.numberFormatCall(ret);
		// }
	...
	*/
		...
		_ExtraCurrentButton.setText(
			typeof _Settings.numberFormatCall == 'function' ? _Settings.numberFormatCall(year + '-' + periodEndYear) : year + '-' + periodEndYear
	...
	*/
		...
		for(year -= 10, length = _ExtraNavigationPeriods.length, i = 0; i < length; i++, year += 10) {
			...
			obj.setText(
				typeof _Settings.numberFormatCall == 'function' ? _Settings.numberFormatCall(year + '-' + (year + 9)) : year + '-' + (year + 9)
				, getDecadesButtonText(year)
				, strLabel
			);
	...
	*/
		...
		var intMonth, intYear, date, monthObj, intLimitMonth, intLimitYear, strCurrentYear;
		...
		strCurrentYear = _longMonthNameArr[intMonth] + ' ' + intYear;
		if(typeof _Settings.numberFormatCall == 'function') {
			strCurrentYear = _Settings.numberFormatCall(strCurrentYear);
		}
		_CurrentMonthButton.setText(strCurrentYear, _Settings.Phrases.getYearBreakDown.replace('{0}', intYear));
	...
	*/
		var height, pos, win, intTop, intLeft
			, offset, padding
			, calendarY1, calendarY2, windowY2
			, calendarX1, calendarX2, windowX2
		;
		
		_hasFadeOutClose = false;
		...
		if(!_isCalendarOpen) {
			...
			if(_Settings.isDropDown) {
				...
				pos = getElementPos(_$calendar[0]);
				win = getWindowDimentions();
				
				padding = 10;
				calendarY1 = pos.y + height;
				calendarY2 = pos.height + calendarY1 + padding;
				windowY2 =  win.scrollTop + win.height;
				
				calendarX1 = pos.x;
				calendarX2 = pos.width + calendarX1 + padding;
				windowX2 =  win.scrollLeft + win.width;
				
				intTop = 0;
				intLeft = 0;
				
				//calculates the Y position of the calendar
				if(calendarY2 > windowY2) {
					intTop = windowY2 - calendarY2;
					if(intTop < 0) {
						_$calendar.css({
							top : intTop + 'px'
						});
						_hasFadeOutClose = true;
					}
					offset = calendarY1 + intTop; //because the top will always be negative here
					if(offset < 0) {
						intTop += Math.abs(offset) + padding;
						if(intTop > 0) {
							intTop = 0;
						}
					}
				...
				//calculates the X position of the calendar
				if(calendarX2 > windowX2) {
					intLeft = windowX2 - calendarX2;
					offset = calendarX1 + intLeft; //because the top will always be negative here
					if(offset < 0) {
						intLeft += Math.abs(offset) + padding;
						if(intLeft > 0) {
							intLeft = 0;
						}
					}
				...
				if(_isExtraNavigatorOpen) {
					if(_ExtraPrevButton.isDisabled()) {
						_ExtraCurrentButton.focus();
					} else {
